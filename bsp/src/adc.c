...
void BSP_ADC_Init(void)
{
    ...
    hdma_adc1.Init.MemInc = DMA_MINC_ENABLE; // Increase array index by one after each measurement
    ...
    sConfig.SamplingTime = ADC_SAMPLETIME_84CYCLES; // Average sample time (stable)

    /* Pin Assignment */
    const uint32_t channels[SENSOR_COUNT] = {
                ADC_CHANNEL_0, ADC_CHANNEL_5, ADC_CHANNEL_6,
...
uint16_t *BSP_ADC_GetValues(void)
{
